...
### バッチスケジューラ
- JST 0, 12時 に自動生成 (1日2回)
- 最大732バッチ保持 (1年分 = 2回/日 × 約365日、1バッチ約4アイデア)
- 1回あたりの生成数・RSS収集数は半分 (`DEFAULT_IDEA_COUNT=4` / RSS記事候補9・表示4)
- 同一スロットのバッチは置き換え
- トレンドキャッシュTTL: 24時間（定時バッチは force 更新のためTTLに関わらず再取得）
