...
| --- | --- | --- |
| Planning tool | Upstream planner | Supplies approved story/task definitions in `.ai/`.
| Coding agent | Implementer | Implements stories, maintains logs, prepares handoffs.
| IDE | Reviewer | Validates code, suggests refactors, runs manual tests.
| Linter | Rule checker | Ensures compliance with repo-specific conventions.
| GitHub MCP Server | Context provider | Bridges GitHub repositories into the agent via MCP tools. |
