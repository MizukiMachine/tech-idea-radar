...
## 各アイデアのJSON形式
{
  ...
  "sources": {
    ...
    "demandSignals": 0,
    "sourceSeedId": "X需要シード由来なら x-demand-1 のようなseedId。RSS/一般知識由来なら省略",
    "evidenceUrls": [
...
- RSS記事やX投稿のURLが入力に含まれる場合は、各アイデアの根拠として入力内の関連URLだけをsources.evidenceUrlsに最大1件入れること
- X投稿は「後付けの引用」ではなく、市場ペインの起点として扱うこと
- X需要シードから生成するアイデアは、その投稿本文の悩み・要望を直接解決するプロダクトにすること
- X需要シードから生成したアイデアはsources.sourceSeedIdに元のseedIdを必ず入れ、sources.evidenceUrlsにはその元投稿URLを必ず1件入れること
- X投稿に直接由来しないアイデアにはX URLを入れないこと
- X需要シードがある場合は、最低5件はX投稿を起点にしたアイデアを含めること。ただし投稿本文が低品質・宣伝・抽象的すぎる場合は無理に使わないこと
- sources.evidenceUrlsには、そのアイデアのcoreProblemやtagsと直接関連する記事・投稿だけを入れること
