...
- Run manually: `docker run --rm -i -e GITHUB_TOKEN -e GITHUB_OWNER=YOUR_OWNER -e GITHUB_REPO=YOUR_REPO github-mcp-server:latest` to forward STDIN/STDOUT while inheriting the exported `GITHUB_TOKEN` from the host shell.
- When using Codex manifests, point the MCP server entry to `docker run --rm -i -e GITHUB_TOKEN -e GITHUB_OWNER=YOUR_OWNER -e GITHUB_REPO=YOUR_REPO github-mcp-server:latest` so the CLI attaches via STDIO.
